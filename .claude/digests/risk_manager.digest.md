...
# Updated 2026-03-10: micro-account-friendly risk settings, floor rounding fix.

## class RiskManager
__init__(storage): holds reference to Storage

validate_trade(direction, lots, entry, stop_loss, take_profit, confidence, balance,
               upcoming_events=None, indicators_snapshot=None, is_scalp=False, setup_type=None) -> dict
  ...
  #   2. margin: lots*1*entry*0.005/balance <= MAX_MARGIN_PERCENT (10%)
  #   3. risk_reward: effective_rr = (reward-7)/(risk+7) >= 1.2 (oversold reversal: 1.0)
  ...
  #   4B. portfolio_risk: total open risk (new + existing) <= MAX_PORTFOLIO_RISK_PERCENT (15%) of balance
  ...
  #   10. dollar_risk: lots*1*risk_pts <= balance * MAX_RISK_PERCENT (8%) — enforced hard cap
  #   11. lot_size: lots >= 0.02 (MIN_LOT_SIZE for Japan 225 Mini)
...
get_safe_lot_size(balance, price, sl_distance, confidence=None, peak_balance=None) -> float
  ...
  # risk_pct = RISK_PERCENT (5.0%) by default
  ...
  # Hard cap by margin: max lots = (balance * 10%) / (price * 0.005)
  # Uses FLOOR rounding (int(lots*100)/100) to avoid exceeding margin cap
  # Always >= MIN_LOT_SIZE (0.02)
...
get_dynamic_sl(atr, setup_type=None, fallback_pts=None) -> float
  ...
  # Always >= SL_FLOOR_PTS (60). Falls back to fallback_pts or DEFAULT_SL_DISTANCE if ATR unavailable.
